...
#--------------------------------------------------------------------
a=input("what do you want to search? ")   #input is a
y1=re.findall(r"\w+",a)  #surch words list
y=list(set(y1))   # remove print(y)
num=[]
num2=[]
if 'or' in y:
    y.remove('or')
    if 'and' in y:
        ...
        print('do AND',y)
        lenth=len(y)
        for j,quote in enumerate(data_list):
            for key_word in y:
               found_at=quote.find(key_word)
               if(found_at>=0):
                   num.append(j)
        for i in num:
            if num.count(i)==lenth:
                num2.append(i)
        num1=list(set(num2))
        print("Found at",num1)
    else:
